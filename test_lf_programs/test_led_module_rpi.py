...
# The logging level for libcamera must be set before importing picamera2, otherwise the logging level will not be applied correctly.
import os
...
def open_camera(index, size=(1280, 720), fmt="RGB888"):
    try:
        cam = Picamera2(index)
    except IndexError:
        raise RuntimeError(f"Camera {index} not found — is it detected? "
                           f"(check: rpicam-hello --list-cameras)")
    except RuntimeError as e:
        # Typical causes: device busy (another process), dma_heap permissions
        raise RuntimeError(f"Camera {index} could not be opened: {e}")

    try:
        config = cam.create_video_configuration(main={"size": size, "format": fmt})
        cam.configure(config)
    except (RuntimeError, ValueError) as e:
        cam.close()
        raise RuntimeError(f"Camera {index}: invalid configuration "
                           f"{size}/{fmt}: {e}")
    ...
    try:
        ...
        cam.close()
        raise RuntimeError(f"Camera {index} failed to start: {e}")
...
def led_detect2(num):
    ...
    cam = open_camera(0, size=HIGH_RESOLUTION, fmt="RGB888")
    if cam is not None:
        flag = 1
    ...
    # cam_index = 0
    # TODO: This part of code is almost done with rework.

    # Pi camera 2 initialization So this part is not needed.
    ...
    try:
        while blink_count < MAX_COUNT:
            try:
                ...
                print("Timeout while capturing frame. Retrying...")
                continue
            except RuntimeError as e:
